...
import 'MotionTabController.dart';
...
///  * [MotionTabBarView], which displays a widget for the currently selected tab.
...
///  * [TabBar], which displays a row of tabs.
///  * [MotionTabBarView], which displays a widget for the currently selected tab.
///  * [MotionTabController], which coordinates tab selection between a [TabBar] and a [MotionTabBarView].
...
class _TabStyle extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    var style = TextStyle(
        ...
        fontSize: ConstantWidget.getWidthPercentSize(context, 1.5),
        fontFamily: ConstantData.fontFamily);

    // To enable TextStyle.lerp(style1, style2, value), both styles must have
    // the same value of inherit. Force that to be inherit=true here.
    final TextStyle defaultStyle =
        (labelStyle ?? tabBarTheme.labelStyle ?? style
    ...
    final Color selectedColor =
        labelColor ?? tabBarTheme.labelColor ?? Colors.black;
...
double _indexChangeProgress(MotionTabController controller) {
...
class _IndicatorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (controller.indexIsChanging) {
      ...
      // The user is dragging the MotionTabBarView's PageView left or right.
...
/// conjunction with a [MotionTabBarView].
///
/// If a [MotionTabController] is not provided, then a [DefaultMotionTabController] ancestor
/// must be provided instead. The tab controller's [MotionTabController.length] must
/// equal the length of the [tabs] list and the length of the
/// [MotionTabBarView.children] list.
///
/// Requires one of its ancestors to be a [Material] widget.
///
/// Uses values from [TabBarTheme] if it is set in the current context.
///
/// To see a sample implementation, visit the [MotionTabController] documentation.
///
/// See also:
///
///  * [MotionTabBarView], which displays page views that correspond to each tab.
class TabBar extends StatefulWidget implements PreferredSizeWidget {
  /// Creates a material design tab bar.
  ///
  /// The [tabs] argument must not be null and its length must match the [controller]'s
  /// [MotionTabController.length].
  ///
  /// If a [MotionTabController] is not provided, then there must be a
  /// [DefaultMotionTabController] ancestor.
  ///
  ...
  ///
  /// The length of this list must match the [controller]'s [MotionTabController.length]
  /// and the length of the [MotionTabBarView.children] list.
  final List<Widget> tabs;

  /// This widget's selection and animation state.
  ///
  /// If [MotionTabController] is not provided, then the value of [DefaultMotionTabController.of]
  /// will be used.
  final MotionTabController controller;
  ...
  /// even if the tap doesn't change the MotionTabController's index. TabBar [onTap]
...
class _TabBarState extends State<TabBar> {
  ...
  void _updateTabController() {
    final MotionTabController newController =
        widget.controller ?? DefaultMotionTabController.of(context);
    assert(() {
      if (newController == null) {
        throw FlutterError('No TabController for ${widget.runtimeType}.\n'
            'When creating a ${widget.runtimeType}, you must either provide an explicit '
            'TabController using the "controller" property, or you must ensure that there '
            'is a DefaultMotionTabController above the ${widget.runtimeType}.\n'
  ...
  void _handleTabControllerAnimationTick() {
    ...
    if (!_controller.indexIsChanging && widget.isScrollable) {
      // Sync the TabBar's scroll position with the MotionTabBarView's PageView.
      _currentIndex = _controller.index;
  ...
  Widget build(BuildContext context) {
    ...
    // If the controller was provided by DefaultMotionTabController and we're part
...
///
/// If a [TabController] is not provided, then there must be a [DefaultMotionTabController]
/// ancestor.
///
...
class MotionTabBarView extends StatefulWidget {
  /// Creates a page view with one child per tab.
  ///
  /// The length of [children] must be the same as the [controller]'s length.
  const MotionTabBarView({
    Key key,
  ...
  /// This widget's selection and animation state.
  ///
  /// If [TabController] is not provided, then the value of [DefaultMotionTabController.of]
  /// will be used.
  final MotionTabController controller;
  ...
  ///
  /// For courier, determines how the page view continues to animate after the
...
class _TabBarViewState extends State<MotionTabBarView> {
  ...
  @override
  void didUpdateWidget(MotionTabBarView oldWidget) {
    super.didUpdateWidget(oldWidget);
...
class TabPageSelector extends StatelessWidget {
  ...
  /// [DefaultMotionTabController.of] will be used.
  final MotionTabController controller;
  ...
  Widget build(BuildContext context) {
    ...
    final MotionTabController tabController =
        controller ?? DefaultMotionTabController.of(context);
    assert(() {
      if (tabController == null) {
        throw FlutterError('No TabController for $runtimeType.\n'
            'When creating a $runtimeType, you must either provide an explicit TabController '
            'using the "controller" property, or you must ensure that there is a '
            'DefaultMotionTabController above the $runtimeType.\n'
            'In this case, there was neither an explicit controller nor a default controller.');
      }
      return true;
    }());
